...
const useDeviceDetector = require('../../build/index');
...
describe('useDeviceDetector', () => {
  ...
  test('should return an object of functions and vals', () => {
    const detect = useDeviceDetector();
...
describe('getDeviceDetect', () => {
  test('should return isMobile true when valid user agent is mobile', () => {
    ...
    expect(useDeviceDetector().isMobile()).toEqual(true);
    ...
    expect(useDeviceDetector().isMobile()).toEqual(true);
    expect(useDeviceDetector().isAndroid()).toEqual(true);
    expect(useDeviceDetector().isIos()).toEqual(false);

    const windows10 = `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 
    (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.246`;
    setUserAgent(windows10);
    expect(useDeviceDetector().isMobile()).toEqual(false);
    expect(useDeviceDetector().isDesktop()).toEqual(true);
    ...
    expect(useDeviceDetector().isMobile()).toEqual(true);
    expect(useDeviceDetector().isIos()).toEqual(true);
    expect(useDeviceDetector().isAndroid()).toEqual(false);
  ...
  test('should return isSSR true when valid user agent is SSR', () => {
    setUserAgent('SSR');
    expect(useDeviceDetector().isSSR()).toEqual(true);
  });
});
